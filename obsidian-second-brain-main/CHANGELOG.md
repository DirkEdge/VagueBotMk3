...
- **Autonomous Conversation Routing, Session Continuation & Passive/Active Listening:** Enabled the bot to autonomously reply without pings in DMs, replies to the bot, in designated channels (`second-brain`, `vague-bot`), or during active conversation sessions (5-minute continuation timeout per channel per user). Implemented a VRAM-optimized Passive/Active split: the bot silently listens and logs the context of all messages (prepending user names in history) in any channel it resides, but only runs the expensive Qwen-Agent inference when explicitly called/triggered. Added a `!clear` utility command to reset active channel history.
...
- **`.env.example` template:** Configuration template containing environment variables for Discord token, Vault paths, and Qwen-Agent LLM configurations.
...
- **XML Tag Normalization & Markdown Code Fence Stripping:** Added robust tool call tag normalization in `nous_fncall_prompt.py` to intercept and convert diverse XML formats (json, toolCall, tool_call, tool_calls, tools, tool, functionCall, function-call, function_call, request, call, execute) into standard `<tool_call>`/`</tool_call>` tags, and automatically strip surrounding markdown code fences (xml, json, etc.) generated by local models like Qwen2.5-Coder under Ollama. This was expanded to auto-wrap code-fenced and raw JSON objects lacking explicit `<tool_call>` tags, ensuring function calls are always correctly parsed and executed autonomously.
- **Clean User History Prompt Metadata:** Refactored `on_message` in `discord_agent_bot.py` to append user messages to conversational history using the native `name` field rather than prepending the username directly to the text content (which confused the LLM's instruction parsing). Cleaned the username to comply with standard LLM naming constraints (`^[a-zA-Z0-9_-]+$`).
- **Expanded Session Timeout:** Increased `SESSION_TIMEOUT_SECONDS` in `discord_agent_bot.py` from 300 (5 minutes) to 600 (10 minutes) to allow users sufficient time to read and approve tool execution gates before conversational sessions expire.
- **Conversation History Sequence Alignment:** Cleaned history building in `discord_agent_bot.py` (`get_history_with_system_context`) to ensure that the conversational messages payload (excluding system messages) always starts with a `user` role message. This prevents Ollama 400 Bad Request errors that occur when history pruning truncates the sequence to start with an `assistant` role message.

- **IDE Static Linter Errors in `obsidian_tools.py`:** Added `# type: ignore` comments to `qwen_agent.tools.base` and dynamic `vault_health` imports to silence false-positive IDE static analysis warnings.
