...
# Thread pool for file I/O to avoid blocking the event loop
_history_executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)

def _write_history_file(json_string):
    """Background task to write the history file atomically."""
    try:
...
def save_histories():
    """Serializes history and offloads file I/O to a background thread to prevent blocking."""
    try:
        # 1. Thread-safe copy & serialize in main thread (prevents "dictionary changed size" errors)
        # Serializing to string is much faster than copy.deepcopy()
        to_save = {str(k): v for k, v in channel_histories.items()}
        json_string = json.dumps(to_save, ensure_ascii=False, indent=2)

        # 2. Offload atomic file write to background thread
        _history_executor.submit(_write_history_file, json_string)
    except Exception as e:
