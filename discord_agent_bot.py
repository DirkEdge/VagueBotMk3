...
async def ping(ctx):
    ...
    await ctx.send(f"🏓 **Pong!** Gateway latency is {latency}ms")
...
def sync_agent_run(agent_instance, messages_list, placeholder=None, bot_loop=None, user_id=None) -> str:
    ...
    try:
        ...
        bot_response_content = f"Error in agent processing: {e}\n\n💡 *Tip: Context bounds may have been reached. Try breaking down your request, or type `!clear` to reset the conversation state.*"
    finally:
