...
# Single-worker ThreadPoolExecutor ensures FIFO ordered writing for background file I/O
...
def save_histories():
    """Optimized: Serialize state in main thread and offload disk write to thread pool."""
    try:
        # Convert keys to strings for JSON serialization
        to_save = {str(k): v for k, v in channel_histories.items()}
        # Serialize to string in main thread to prevent thread-safety issues (size changed during iteration)
        json_str = json.dumps(to_save, ensure_ascii=False, indent=2)
        # Offload slow synchronous I/O
