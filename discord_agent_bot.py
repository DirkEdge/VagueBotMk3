...
# Use a single-worker ThreadPoolExecutor to ensure sequential file writes (FIFO order)
...
def _write_history_to_disk(json_str):
    """Helper function to perform the actual disk write synchronously."""
    try:
        # Write to a temporary file first, then rename to ensure atomic write
        # and prevent file corruption if interrupted.
...
def save_histories():
    try:
        # Optimization: Prepare data payload synchronously in main thread to avoid race conditions.
        # We serialize to JSON string here in the main thread to ensure we don't pass
        # shared mutable state to the background thread.
        ...
        logger.error(f"Error dispatching chat histories save: {e}")
