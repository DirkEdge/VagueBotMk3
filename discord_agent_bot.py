...
# Single-worker executor guarantees FIFO order for I/O operations
...
def _write_history_to_disk(json_data: str):
...
def save_histories():
    try:
        # Convert keys to strings and serialize in main thread to avoid thread-safety issues during iteration
        to_save = {str(k): v for k, v in channel_histories.items()}
        json_data = json.dumps(to_save, ensure_ascii=False, indent=2)
        # Offload the blocking disk write to the background executor
        _io_executor.submit(_write_history_to_disk, json_data)
    except Exception as e:
