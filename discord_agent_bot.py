...
# Single-worker thread pool executor to offload file I/O and prevent blocking the main event loop
# A single worker guarantees FIFO execution to keep state writes sequential and safe
...
def _write_history_to_disk(json_str: str):
    """Background worker function for writing chat history atomicity."""
    try:
        tmp_file = f"{HISTORY_FILE}.tmp"
        # Atomic write: write to temp file then replace, preventing data corruption on crash
...
def save_histories():
    """Serialize the chat histories safely on the main thread and offload disk write."""
    try:
        # ⚡ Bolt optimization: Serialize the state to a string in the main thread to ensure thread safety
        # This prevents "dictionary changed size during iteration" errors without needing deepcopy
