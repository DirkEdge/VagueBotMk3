...
# Single-worker executor to offload disk I/O and guarantee FIFO ordering
...
def save_histories():
    try:
        ...
        # Serialize on the main thread to avoid 'dictionary changed size during iteration' thread safety issues
        json_str = json.dumps(to_save, ensure_ascii=False, indent=2)
        # Offload file I/O to background thread to prevent blocking the async event loop
