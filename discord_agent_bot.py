...
# Offload I/O to a background thread to avoid blocking the main event loop
_io_executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)

def _save_histories_worker(json_data: str, history_file: str):
    try:
...
def save_histories():
    # Optimization: Offload blocking I/O to a background thread to prevent asyncio event loop stalling.
    # Using json.dumps in main thread ensures thread safety (state snapshot).
    # Atomic write prevents corruption.
    try:
        to_save = {str(k): v for k, v in channel_histories.items()}
        json_data = json.dumps(to_save, ensure_ascii=False, indent=2)
        _io_executor.submit(_save_histories_worker, json_data, HISTORY_FILE)
    except Exception as e:
