...
# Background executor for non-blocking, atomic file writes (FIFO order)
...
def _write_history_to_disk(json_str: str):
    ...
    try:
        ...
        logger.error(f"Error in background history write: {e}")
...
def save_histories():
    """Serializes history on the main thread and offloads file I/O to a background thread to prevent blocking the event loop."""
    try:
        # Convert keys to strings for JSON serialization
        to_save = {str(k): v for k, v in channel_histories.items()}
        # Serialize to string in main thread to avoid concurrent modification issues in background thread
        json_str = json.dumps(to_save, ensure_ascii=False, indent=2)
        # Offload the blocking disk write to the single-worker executor
